...
// UI taps: the footstep scuff pitched up makes a decent click.
static void tapMove()   { Audio::Play(RK::SND_FOOTSTEP_STOP, 0.5f, 1.9f); }
static void tapSelect() { Audio::Play(RK::SND_FOOTSTEP_STOP, 0.8f, 1.4f); }
...
bool Game::handleMenus()
{
    ...
    case Screen::MainMenu:
    {
        ...
        if (activate)
        {
            ...
            case 3: setScreen(Screen::Credits); break;
            }
        }
    ...
    case Screen::NameEntry:
    {
        ...
        if (IsKeyPressed(KEY_ESCAPE)) { tapSelect(); setScreen(Screen::PlayMenu); }
        break;
    }
...
{
    // Thumbnail full-screen, nothing else but the blinking prompt.
    ...
    if (fmodf((float)GetTime(), 1.2f) < 0.8f)
    {
        ...
        // soft band so the text reads on any photo
        DrawRectangle(x - 20, y - 10, w + 40, 56, ColorAlpha(BLACK, 0.5f));
        DrawText(prompt, x, y, 36, RAYWHITE);
    }
}
